...
public class SkeletronHandRenderer extends GeoNormalRenderer<SkeletronHand> {
    ...
    @Override
    public void preRender(PoseStack poseStack, SkeletronHand animatable, BakedGeoModel model, @Nullable MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float rr, float gg, float bb, float aa) {
...
//        poseStack.translate(0, 0.5f, 0);
        ...
        if(animator == null){
            ...
            bone.arm2 = model.getBone("arm2").get().getChildBones().get(0);
        ...
        super.preRender(poseStack, animatable, model, bufferSource, buffer, isReRender, partialTick, packedLight, packedOverlay, rr, gg, bb, aa);
    ...
    @Override
    protected void applyRotations(SkeletronHand animatable, PoseStack poseStack, float ageInTicks, float rotationYaw, float partialTick) {
//        poseStack.translate(-11.4f, -0.6f, 0);
