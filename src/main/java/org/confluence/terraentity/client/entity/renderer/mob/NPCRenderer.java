...
//            target.blitToScreen(200,200); // debug
    ...
    private void renderChatBubble(Matrix4f viewMatrix, TextureTarget target, float progress, int light) {
        ...
        consumer.vertex(viewMatrix, -0.5f, -0.5f, 0).uv(1, 0).endVertex();
        consumer.vertex(viewMatrix, 0.5f, -0.5f, 0).uv(0, 0).endVertex();
        consumer.vertex(viewMatrix, 0.5f, 0.5f, 0).uv(0, 1).endVertex();
        consumer.vertex(viewMatrix, -0.5f, 0.5f, 0).uv(1, 1).endVertex();
