...
public class PixieRenderer<T extends Pixie> extends GeoNegativeVolumeRenderer<T> {
    ...
    public void actuallyRender(PoseStack poseStack, T animatable, BakedGeoModel model, @Nullable RenderType renderType,
        ...
        if(isReRender){
            super.actuallyRender(poseStack, animatable, model, renderType, bufferSource, buffer, true, partialTick, packedLight, packedOverlay, 0.81f,0.81f,0.81f,0.81f);
        }else{
            super.actuallyRender(poseStack, animatable, model, renderType, bufferSource, buffer, false, partialTick, 0X0F000F0, packedOverlay, 1,1,1,1);
        }
    }
}
