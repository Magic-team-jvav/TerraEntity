...
//                                                BTServer.startServer(Files.readAllBytes(Paths.get("D:\\programingFiles\\java\\untitled3\\src\\main\\resources\\behavior_tree_viewer.html")));
                                                ...
                                                String text = "http://localhost:" + ServerConfig.BEHAVIOR_TREE_WEB_VIEWER_SERVER_PORT.get();
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand usedHand) {
        if (!player.isShiftKeyDown()) {
            return player.getCapability(TEAttachments.UNSYNC).orElse(new UnSyncableAttachment()).getDebugMode().apply(level, player, usedHand);
        }
